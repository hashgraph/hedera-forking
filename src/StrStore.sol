...
Vm constant vm_ = Vm(address(uint160(uint256(keccak256("hevm cheat code")))));
...
function storeString(address target, uint256 slot, string memory strvalue) {
    ...
    if (length <= 31) {
        ...
        vm_.store(target, bytes32(slot), slotValue);
        return;
    }

    vm_.store(target, bytes32(slot), bytes32(length * 2 + 1));
    ...
    for (uint256 i = 0; i < numChunks; i++) {
        ...
        vm_.store(target, bytes32(uint256(baseSlot) + i), chunk);
    }
}
