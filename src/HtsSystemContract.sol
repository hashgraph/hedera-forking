...
contract HtsSystemContract is NoDelegateCall, KeyHelper {
    ...
    mapping(address => IHederaTokenService.TokenInfo) internal _nftTokenInfos;
    // NFT token -> serialNumber -> PartialNonFungibleTokenInfo
    mapping(address => mapping(int64 => HederaTokenValidation.PartialNonFungibleTokenInfo)) internal _partialNonFungibleTokenInfos;
    ...
    mapping(address => mapping(address => HederaTokenValidation.TokenConfig)) internal _kyc; // is KYCed is the positive case(i.e. explicitly requires KYC approval); see defaultKycStatus
    // HTS token -> account -> isFrozen
    mapping(address => mapping(address => HederaTokenValidation.TokenConfig)) internal _unfrozen; // is unfrozen is positive case(i.e. explicitly requires being unfrozen); see freezeDefault
    ...
    function _setFungibleTokenInfoToken(address token, IHederaTokenService.HederaToken memory hederaToken) internal {
    ...
    function _setFungibleTokenExpiry(address token, IHederaTokenService.Expiry memory expiryInfo) internal {
    ...
    function _setFungibleTokenInfo(address token, IHederaTokenService.TokenInfo memory tokenInfo) internal {
    ...
    function _setFungibleTokenKeys(address token, IHederaTokenService.TokenKey[] memory tokenKeys) internal {
    ...
    function _setFungibleTokenInfo(IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo) internal returns (address treasury) {
    ...
    function _setNftTokenInfoToken(address token, IHederaTokenService.HederaToken memory hederaToken) internal {
    ...
    function _setNftTokenExpiry(address token, IHederaTokenService.Expiry memory expiryInfo) internal {
    ...
    function _setNftTokenInfo(address token, IHederaTokenService.TokenInfo memory nftTokenInfo) internal {
    ...
    function _setNftTokenKeys(address token, IHederaTokenService.TokenKey[] memory tokenKeys) internal {
        // Copy the tokenKeys array
        uint256 length = tokenKeys.length;
        for (uint256 i = 0; i < length; i++) {
            IHederaTokenService.TokenKey memory tokenKey = tokenKeys[i];
    ...
    function _setNftTokenInfo(IHederaTokenService.TokenInfo memory nftTokenInfo) internal returns (address treasury) {
        address tokenAddress = msg.sender;
    ...
    function _precheckCreateToken(
        ...
        IHederaTokenService.KeyValue memory key = _getTokenKey(token.tokenKeys, _getKeyTypeValue(KeyHelper.KeyType.ADMIN));
        ...
        for (uint256 i = 0; i < token.tokenKeys.length; i++) {
            IHederaTokenService.TokenKey memory tokenKey = token.tokenKeys[i];
    ...
    function _precheckUpdateTokenExpiryInfo(address sender, address token, IHederaTokenService.Expiry memory expiryInfo) internal view returns (bool success, int64 responseCode) {
        success = true;
        (success, responseCode) = success ? HederaTokenValidation._validateToken(token, _tokenDeleted, _isFungible, _isNonFungible) : (success, responseCode);
    ...
    function _precheckUpdateTokenInfo(address sender, address token, IHederaTokenService.HederaToken memory tokenInfo) internal view returns (bool success, int64 responseCode) {
        success = true;
        (success, responseCode) = success ? HederaTokenValidation._validateToken(token, _tokenDeleted, _isFungible, _isNonFungible) : (success, responseCode);
        (success, responseCode) = success ? _validateAdminKey(token) : (success, responseCode);
    ...
    function _precheckUpdateTokenKeys(address sender, address token, IHederaTokenService.TokenKey[] memory keys) internal view returns (bool success, int64 responseCode) {
        success = true;
        (success, responseCode) = success ? HederaTokenValidation._validateToken(token, _tokenDeleted, _isFungible, _isNonFungible) : (success, responseCode);
        (success, responseCode) = success ? _validateAdminKey(token) : (success, responseCode);
    ...
    function _doesAccountPassUnfrozen(int64 responseCode, bool isFrozen_) internal pure returns (bool doesPassUnfrozen) {
        doesPassUnfrozen = responseCode == HederaResponseCodes.SUCCESS ? !isFrozen_ : true;
    }
    ...
    function _postMint(
        ...
        if (_isNonFungible[token]) {
            _partialNonFungibleTokenInfos[token][amountOrSerialNumber] = HederaTokenValidation.PartialNonFungibleTokenInfo({
    ...
    function registerHederaFungibleToken(address caller, IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo) external {
    ...
    function registerHederaNonFungibleToken(address caller, IHederaTokenService.TokenInfo memory nftTokenInfo) external {
    ...
    function getFungibleTokenInfo(
        address token
    ) external view returns (int64 responseCode, IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo) {
    ...
    function getNonFungibleTokenInfo(
        address token,
        int64 serialNumber
    ) external view returns (int64 responseCode, IHederaTokenService.NonFungibleTokenInfo memory nonFungibleTokenInfo) {
        ...
        // TODO: abstract logic into _get{Data} function
        IHederaTokenService.TokenInfo memory nftTokenInfo = _nftTokenInfos[token];
        HederaTokenValidation.PartialNonFungibleTokenInfo memory partialNonFungibleTokenInfo = _partialNonFungibleTokenInfos[token][
            serialNumber
        ];
    ...
    function getTokenExpiryInfo(address token) external view returns (int64 responseCode, IHederaTokenService.Expiry memory expiry) {
    ...
    function getTokenKey(address token, uint keyType) external view returns (int64 responseCode, IHederaTokenService.KeyValue memory key) {
    ...
    function _getTokenKey(IHederaTokenService.TokenKey[] memory tokenKeys, uint keyType) internal view returns (IHederaTokenService.KeyValue memory key) {
        uint256 length = tokenKeys.length;
    ...
    function isFrozen(address token, address account) public view returns (int64 responseCode, bool frozen) {
        ...
        bool freezeDefault;
        if (isFungible) {
            IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo = _fungibleTokenInfos[token];
        ...
        HederaTokenValidation.TokenConfig memory unfrozenConfig = _unfrozen[token][account];
    ...
    function isKyc(address token, address account) public view returns (int64 responseCode, bool kycGranted) {
        ...
        HederaTokenValidation.TokenConfig memory kycConfig = _kyc[token][account];
    ...
    function isToken(address token) public view returns (int64 responseCode, bool isToken_) {
        isToken_ = _isToken(token);
        responseCode = isToken_ ? HederaResponseCodes.SUCCESS : HederaResponseCodes.INVALID_TOKEN_ID;
    }
    ...
    function allowance(
        ...
        address owner,
        address spender
    ) public view returns (int64 responseCode, uint256 allowance_) {

        bool success;
        (success, responseCode) = _precheckAllowance(token, owner, spender);

        if (!success) {
            return (responseCode, allowance_);
        }

        // TODO: abstract logic into _allowance function
        allowance_ = IERC20(token).allowance(owner, spender);
    }
    ...
    function createFungibleToken(
        ...
        TokenProxy hederaFungibleToken = new TokenProxy();
    ...
    function createNonFungibleToken(
        ...
        TokenProxy hederaNonFungibleToken = new TokenProxy();
    ...
    function dissociateTokens(
        address account,
        address[] memory tokens
    ) external noDelegateCall returns (int64 responseCode_) {
        for (uint256 i = 0; i < tokens.length; i++) {
