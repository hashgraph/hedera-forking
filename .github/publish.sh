...
modified_test_summary=$(echo "$test_summary" | awk -v line4="$test_summary_line4" 'NR==2 {$0=line4} 1' | awk -v sep="$test_summary_line4" '$0==sep{if(!found){found=1;print;}next;}{print}')
...
modified_test_coverage=$(echo "$test_coverage" | awk -v line4="$test_coverage_line4" 'NR==2 {$0=line4} 1' | awk -v sep="$test_coverage_line4" '$0==sep{if(!found){found=1;print;}next;}{print}')
